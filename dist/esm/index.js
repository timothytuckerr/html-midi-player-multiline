...
class VisualizerElement extends HTMLElement {
    ...
    async initVisualizerNow() {
        ...
        if (this.type === 'piano-roll') {
            ...
            this.visualizers = [new PianoRollSVGVisualizer(this.ns, svg, this._config)];
        }
        else if (this.type === 'waterfall') {
            this.wrapper.classList.add('waterfall-visualizer');
            this.visualizers = [new WaterfallSVGVisualizer(this.ns, this.wrapper, this._config)];
        }
            ...
            for (let i = 0; i < this.ns.notes.length; i += chunkSize) {
                ...
                new_ns.notes = chunk;
                this.visualizers.push(new StaffSVGVisualizer(new_ns, div, this._config));
            }
        }
    }
    reload() {
