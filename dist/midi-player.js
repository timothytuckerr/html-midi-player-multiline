...
(function (global, factory) {
  ...
  class VisualizerElement extends HTMLElement {
    ...
    initVisualizerNow() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (_this.type === 'piano-roll') {
          ...
          _this.visualizers = [new mm.PianoRollSVGVisualizer(_this.ns, svg, _this._config)];
        } else if (_this.type === 'waterfall') {
          _this.wrapper.classList.add('waterfall-visualizer');

          _this.visualizers = [new mm.WaterfallSVGVisualizer(_this.ns, _this.wrapper, _this._config)];
        } else if (_this.type === 'staff') {
          _this.wrapper.classList.add('staff-visualizer');

          _this.visualizers = [];
          var chunkSize = Math.ceil(_this.ns.notes.length / _this.lines);

          var _loop = function (i) {
            var chunk = structuredClone(_this.ns.notes.slice(i, i + chunkSize));
            var startTime = chunk[0].startTime;
            chunk.forEach(n => {
            ...
            var new_ns = structuredClone(_this.ns);
            new_ns.notes = chunk;

            _this.visualizers.push(new mm.StaffSVGVisualizer(new_ns, div, _this._config));
          };

          for (var i = 0; i < _this.ns.notes.length; i += chunkSize) {
            _loop(i);
          }
        }
      })();
    ...
    redraw(activeNote) {
      if (this.visualizers) {
        if (this.type == "staff") {
          var chunkIndex = Math.floor(this.ns.notes.indexOf(activeNote) / Math.ceil(this.ns.notes.length / this.lines));
          ...
          var note = structuredClone(activeNote);
    ...
    get lines() {
      var lines = Number(this.getAttribute('lines'));
